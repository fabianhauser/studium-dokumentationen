...
$LL = L^2$, $L^n = L^{n-1}L, L^0 = \{\epsilon\}$


Für Wortkombinationen wird ein NEA benötigt, welcher die Automaten der einzelnen Sprachen mit $\epsilon$-Übergängen verknüpft.
...
\paragraph{Beispiel}
$\{w \in \epsilon^\ast | w \text{ ist ein Palindrom }\}$ nicht regulär, $\epsilon = \{0,1\}$
...
\[
 \lambda : Q \times (\Sigma \cup \{\epsilon\}) \rightarrow P(Q)
\]

$\cup \{\epsilon\}$ ermöglich Sprünge; es können "Pfeile" weggelassen werden und es können mehrere Pfeile für den gleichen Buchstaben geben.
...
\subsection{$\text{NEA}_\epsilon \to \text{DEA}$}

$\epsilon$-Übergänge sind "gratis", d.h. verbrauchen kein Zeichen.
...
\begin{tabular}{ l l}
	...
	$\epsilon$ (leeres Wort) & steht für $L("") = \{ \epsilon \}$
\end{tabular}
...
\begin{itemize}
	\item Startzustand un Endzustand mittels $\epsilon$-Übergängen hinzufügen.
...
\begin{enumerate}
	\item $\epsilon$ \hfill $A \to \epsilon$
